...
struct	pentry	{
	...
	int pinh;	//inherited prio. This value can be 0 when the process is running with its original priority.
	...
	int	pwaitret;
	int ltable[NLOCKS];	//bitmap. All the lockes held by the proc can be found.
	int waitlock;	//the lockid this proc is waiting
	int plockret;
	//unsigned long lwaitstarttime;
};
